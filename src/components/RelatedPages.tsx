...
export const RelatedPages: React.FC<RelatedPagesProps> = ({ currentBundle, currentNetwork, className = '', onNavigate }) => {
  const getRelatedPages = () => {
    ...
    const staticPages = [
      { name: 'Cheapest Data in South Africa', url: '/guides/cheapest-1gb-data-south-africa/', page: 'guide', slug: 'cheapest-1gb-data-south-africa' },
      { name: 'USSD Code Directory', url: '/ussd-codes-south-africa/', page: 'ussd', slug: undefined },
      { name: 'Vodacom vs MTN Data Prices', url: '/guides/vodacom-vs-mtn-data-prices/', page: 'guide', slug: 'vodacom-vs-mtn-data-prices' }
    ] as const;
    ...
    <div className={`mt-20 pt-10 border-t border-slate-100 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {dynamic.map((bundle: Bundle) => {
          const networkSlug = bundle.network.toLowerCase().replace(/[^a-z0-9]/g, '');
          return (
            <a
              key={bundle.id}
              href={`/network/${networkSlug}/`}
              onClick={(e) => { e.preventDefault(); onNavigate('network', networkSlug); }}
        ...
        {staticPages.map((page) => (
          <a
            key={page.url}
            href={page.url}
            onClick={(e) => { e.preventDefault(); onNavigate(page.page as any, page.slug); }}
