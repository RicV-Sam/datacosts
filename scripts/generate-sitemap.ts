...
function buildRedirectsFile(): string {
  const lines = REDIRECT_ALIASES.map((alias) => `${alias.from} ${alias.to} 301`);
  return [
    '# Generated by scripts/generate-sitemap.ts',
    '# Canonical URLs stay in XML sitemaps; these aliases recover common short or legacy paths.',
    ...lines,
    ''
  ].join('\n');
}
...
async function main(): Promise<void> {
  ...
  console.log(`Generated ${REDIRECT_ALIASES.length} redirect aliases at ${redirectsPath}.`);
}
