...
test('tracker supports future sizes while publishing an immutable August snapshot', () => {
  assert.deepEqual(TRACKED_DATA_SIZES_GB, [5, 10, 15, 20, 30, 50]);
  assert.equal(monthlyDealHistory.length, 1);
  assert.equal(currentMonthlyDealSnapshot.month, '2026-08');
  assert.equal(currentMonthlyDealSnapshot.checkedAt, '2026-08-04');
...
test('derived calculations keep anytime and advertised totals separate', () => {
  const offer = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'vodacom-prepaid-lte-10-plus-10-2026-08');
  assert.ok(offer);
  ...
  const conditional = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'capitec-connect-10gb-30-day-2026-08');
...
test('daily-release and streaming data never become anytime data', () => {
  const daily = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'telkom-daily-dose-30gb-2026-08');
  const streaming = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'mtn-entertainment-streaming-20gb-2026-08');
  assert.ok(daily);
  assert.ok(streaming);
...
test('size bands deterministically include pooled-anytime and advertised-total classes', () => {
  const vodacom20Advertised = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'vodacom-prepaid-lte-10-plus-10-2026-08');
  const vodacom40Advertised = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'vodacom-prepaid-lte-20-plus-20-2026-08');
  const fnb25 = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'fnb-connect-data-plan-25gb-2026-08');
  const standardBank35 = currentMonthlyDealSnapshot.offers.find((row) => row.id === 'standard-bank-connect-connected-gigs-pro-35gb-2026-08');
...
test('ranking keeps cheapest genuine anytime, unit value and advertised price separate', () => {
  ...
  assert.equal(ten.bestOverall?.id, 'vodacom-prepaid-lte-10-plus-10-2026-08');
  assert.equal(twenty.bestOverall?.id, 'standard-bank-connect-connected-gigs-plus-20gb-2026-08');
  assert.equal(thirty.bestOverall?.id, 'mtn-super-data-30gb-2026-08');
  assert.equal(ten.bestAnytimeValue?.id, 'vodacom-prepaid-lte-10-plus-10-2026-08');
  assert.equal(twenty.bestAnytimeValue?.id, 'fnb-connect-data-plan-25gb-2026-08');
  assert.equal(thirty.bestAnytimeValue?.id, 'standard-bank-connect-connected-gigs-pro-35gb-2026-08');
  assert.equal(ten.lowestAdvertisedPrice?.id, 'vodacom-prepaid-lte-5-plus-5-2026-08');
  assert.equal(twenty.lowestAdvertisedPrice?.id, 'vodacom-prepaid-lte-10-plus-10-2026-08');
  assert.equal(thirty.lowestAdvertisedPrice?.id, 'cell-c-day-by-day-30gb-2026-08');
});
